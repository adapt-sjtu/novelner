...
def convert_to_iob1(from_format, prec, curr, succ):
    global inside
    ...
    if from_format == 'IOB1':
        # Single
            # curr_tag == 'B':
            #    Preceeded by SAME TYPE (B or I) (SAME TYPE case)
        ...
        if (curr_tag == 'B' and curr_type == prec_type and
           ((succ_tag == 'B' and curr_type == succ_type) or (succ_tag in {'B', 'O', ''} and curr_type != succ_type))):
            return 'B-{0}'.format(curr_type)
            # curr_tag == 'I':
            #    Preceeded by DIFFERENT TYPE (B or I or sentence boundary) (DIFFERENT TYPE case)
            #    Succeeded by B with SAME TYPE (including Single) or ANY with DIFFERENT TYPE (or O or sentence boundary)
        if (curr_tag == 'I' and curr_type != prec_type and
        ...
        if curr_tag == 'B' and prec_tag in {'B', 'I'} and curr_type == prec_type and succ_tag == 'I' and curr_type == succ_type:
            return 'B-{0}'.format(curr_type)
            # curr_tag == 'I':
            #    Preceeded by E, O with DIFFERENT TYPE (DIFFERENT TYPE case)
            #    Succeeded by anything... (including sentence boundary) (Pedantic: Only by I with SAME TYPE else Single)
        if curr_tag == 'I' and curr_type != prec_type and succ_tag == 'I' and curr_type == succ_type:
            return 'I-{0}'.format(curr_type)
        ...
        if (curr_tag == 'I' and prec_tag in {'B', 'I'} and curr_type == prec_type and
           ((succ_tag == 'B' and curr_type == succ_type) or (curr_type != succ_type or succ_tag == ''))):
            return 'I-{0}'.format(curr_type)
        if curr_tag == 'O':
            return 'O'
    ...
    if from_format == 'IOB2':
        # Single
            # curr_tag == 'B':
            #    Preceeded by anything... (including sentence boundary) (SAME TYPE case)
            #    Succeeded by B (anytype) or O or sentence boundary
        if curr_tag == 'B' and succ_tag in {'B', 'O', ''} and curr_type == prec_type:
            return 'B-{0}'.format(curr_type)
            # curr_tag == 'B':
            #    Preceeded by anything... (including sentence boundary) (DIFFERENT TYPE case)
            #    Succeeded by B (anytype) or O or sentence boundary
        if curr_tag == 'B' and succ_tag in {'B', 'O', ''} and curr_type != prec_type:
            return 'I-{0}'.format(curr_type)
        ...
        # Begin of chunk by tag
            ...
            #    Preceeded by anything... (including sentence boundary) (SAME TYPE case)
            #    Succeeded by anything... (Pedantic: Only by I with SAME TYPE else it is Single)
        if curr_tag == 'B' and curr_type == prec_type:
            return 'B-{0}'.format(curr_type)
            # curr_tag == 'B':
            #    Preceeded by anything... (including sentence boundary) (DIFFERENT TYPE case)
            #    Succeeded by anything... (Pedantic: Only by I with SAME TYPE else it is Single)
    ...
    if from_format == 'IOE1':
        # Single
            # curr_tag == 'I':
            #    Preceeded by DIFFERENT TYPE (or O or sentence boundary) (DIFFERENT TYPE case)
            #    Succeeded by I (or O) with DIFFERENT TYPE (including sentence boundary)
        if ((curr_tag == 'I' and curr_type != prec_type and curr_type != succ_type) or
            # curr_tag == 'E':
            #    Preceeded by DIFFERENT TYPE (or O or sentence boundary) (DIFFERENT TYPE case)
            #    Succeeded by I (or O) with SAME TYPE (including sentence boundary)
                (curr_tag == 'E' and curr_type != prec_type and curr_type == succ_type)):
            return 'I-{0}'.format(curr_type)
            # curr_tag == 'E':
            #    Preceeded by E with SAME TYPE (SAME TYPE case)
            #    Succeeded by SAME TYPE (I or E)
        if ((curr_tag == 'E' and prec_tag == 'E' and curr_type == prec_type and curr_type == succ_type) or
            # curr_tag == 'I':
            #    Preceeded by DIFFERENT TYPE (or O or sentence boundary) (DIFFERENT TYPE case)
            #    Succeeded by I (or O) with ANY TYPE (including sentence boundary)
           (curr_tag == 'I' and prec_tag == 'E' and curr_type == prec_type and curr_type != succ_type)):
            return 'B-{0}'.format(curr_type)
        ...
        if curr_tag == 'I' and curr_type != prec_type and curr_type == succ_type:
            ...
            #    (SAME TYPE case)
            #    Succeeded by ANY with SAME TYPE (I or E)
        if curr_tag == 'I' and prec_tag == 'E' and curr_type == prec_type and curr_type == succ_type:
            return 'B-{0}'.format(curr_type)
    ...
    if from_format == 'IOE2':
        ...
        if curr_tag == 'E' and prec_tag in {'E', 'O', ''} and curr_type == prec_type:
            return 'B-{0}'.format(curr_type)
            # curr_tag == 'E':
            #    Preceeded by E or O with ANY TYPE or sentence boundary (as every end is marked with E)
            #    (DIFFERENT TYPE case)
        ...
        # Begin of chunk by tag
            # curr_tag == 'I':
            #    Preceeded by E or O with ANY TYPE or sentence boundary (SAME TYPE case)
            #    Succeeded by anything... (as every end is marked with E)
        if curr_tag == 'I' and prec_tag in {'E', 'O', ''} and curr_type == prec_type:
            return 'B-{0}'.format(curr_type)
            # curr_tag == 'I':
            #    Preceeded by E or O with ANY TYPE or sentence boundary (DIFFERENT TYPE case)
            #    Succeeded by anything... (as every end is marked with E)
        if curr_tag == 'I' and prec_tag in {'E', 'O', ''} and curr_type != prec_type:
            return 'I-{0}'.format(curr_type)
    ...
    if from_format in {'IOBES', 'SBIEO'}:
        ...
        if curr_tag == 'S' and prec_tag in {'E', 'O', 'S', ''} and succ_tag in {'B', 'O', 'S', ''} and curr_type == prec_type:
            return 'B-{0}'.format(curr_type)
            # curr_tag == 'S':
            #    Preceeded by E or O with ANY TYPE or sentence boundary (DIFFERENT TYPE case)
            #    Succeeded by B or O with ANY TYPE or sentence boundary
        ...
        # Begin of chunk by tag
            ...
            #    Preceeded by E or O with ANY TYPE or sentence boundary (SAME TYPE case)
            #    Succeeded by I or E with SAME TYPE
        if (curr_tag == 'B' and prec_tag in {'E', 'O', 'S', ''} and succ_tag in {'I', 'E'} and curr_type == succ_type and
           curr_type == prec_type):
            return 'B-{0}'.format(curr_type)
        # Begin of chunk by tag
            # curr_tag == 'B':
    ...
    if from_format == 'O+C':
        ...
        if (curr_tag == 'S' and prec_tag in {'E', 'O', 'S', ''} and succ_tag in {'B', 'O', 'S', ''} and
                curr_type == prec_type and not inside):
            return 'B-{0}'.format(curr_type)
            # curr_tag == 'S':
            #    Preceeded by E or O with ANY TYPE or sentence boundary (DIFFERENT TYPE case)
            #    Succeeded by B or O with ANY TYPE or sentence boundary
        ...
        if (curr_tag == 'B' and prec_tag in {'E', 'O', 'S', ''} and ((succ_tag == 'E' and curr_type == succ_type) or
                succ_tag == 'O') and curr_type == prec_type and not inside):
            inside = True
            return 'B-{0}'.format(curr_type)
        # Begin of chunk by tag
            # curr_tag == 'B':
...
if __name__ == '__main__':
    ...
    for l in fh:
        ...
        if len(l) > 0:
            ...
            suc_label = ''
        if len(cur) > 0:
            print(''.join(cur), convert_to_iob1(iob_format, prec_label, cur_label, suc_label))
        else:
            print()
