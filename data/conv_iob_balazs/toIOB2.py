...
def convert_to_iob2(from_format, prec, curr, succ):
    global inside
    ...
    if from_format == 'IOB2':
        ...
        # Begin of chunk by tag
            ...
            #    Preceeded by anything... (including sentence boundary)
            #    Succeeded by anything... (Pedantic: Only by I with SAME TYPE else it is Single)
        if curr_tag == 'B' and succ_tag == 'I' and curr_type == succ_type:
            return 'B-{0}'.format(curr_type)
        ...
        if curr_tag == 'I' and curr_type == prec_type and succ_tag in {'B', 'O', ''}:
            return 'I-{0}'.format(curr_type)
        if curr_tag == 'O':
            return 'O'
        if curr_tag == 'O':
    ...
    if from_format == 'IOE1':
        ...
        if ((curr_tag == 'I' and ((prec_tag in {'I', 'O', ''} and curr_type != prec_type) or
            (prec_tag == 'E' and curr_type == prec_type)) and succ_tag in {'I', 'O', ''} and curr_type != succ_type) or
            # curr_tag == 'E':
    ...
    if from_format == 'O+C':
        ...
        if (curr_tag == 'B' and prec_tag in {'E', 'O', 'S', ''} and ((succ_tag == 'E' and
                curr_type == succ_type and not inside) or succ_tag == 'O')):
            inside = True
            return 'B-{0}'.format(curr_type)
...
if __name__ == '__main__':
    ...
    for l in fh:
        ...
        if len(l) > 0:
            ...
            suc_label = ''
        if len(cur) > 0:
            print(''.join(cur), convert_to_iob2(iob_format, prec_label, cur_label, suc_label))
        else:
            print()
