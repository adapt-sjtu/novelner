...
def convert_to_ioe1(from_format, prec, curr, succ):
    global inside
    ...
    if from_format == 'IOB1':
        # Single
            ...
            #    (DIFFERENT TYPE case)
        if curr_tag == 'B' and curr_type == prec_type and (curr_type != succ_type or succ_tag == ''):
            ...
            #    Succeeded by B with SAME TYPE (including Single) or ANY with DIFFERENT TYPE (or O or sentence boundary)
            #    (SAME TYPE case)
        if curr_tag == 'I' and curr_type != prec_type and succ_tag == 'B' and curr_type == succ_type:
        ...
        # End of chunk by tag
            ...
            #    (Pedantic: Only by B with SAME TYPE or DIFFERENT TYPE else Inside) (DIFFERENT TYPE case)
        if curr_tag == 'I' and curr_type == prec_type and curr_type != succ_type:
            return 'I-{0}'.format(curr_type)
            # curr_tag == 'I':
            #    Preceeded by SAME TYPE (B or I)
            #    Succeeded by anything... (including sentence boundary)
            #    (Pedantic: Only by B with SAME TYPE or DIFFERENT TYPE else Inside) (SAME TYPE case)
        if curr_tag == 'I' and curr_type == prec_type and curr_type == succ_type:
            return 'E-{0}'.format(curr_type)
        if curr_tag == 'O':
            return 'O'
    ...
    if from_format == 'IOB2':
        # Single
            ...
            #    Succeeded by B (anytype) or O or sentence boundary (DIFFERENT TYPE case)
        if curr_tag == 'B' and succ_tag in {'B', 'O', ''} and curr_type != succ_type:
            return 'I-{0}'.format(curr_type)
            # curr_tag == 'B':
            #    Preceeded by anything... (including sentence boundary)
            #    Succeeded by B (anytype) or O or sentence boundary (SAME TYPE case)
        if curr_tag == 'B' and succ_tag in {'B', 'O', ''} and curr_type == succ_type:
            return 'E-{0}'.format(curr_type)
        ...
        # End of chunk by tag
            # curr_tag == 'I':
            #    Preceeded by anything... (Pedantic: Only with SAME TYPE else it is Invalid)
            #    Succeeded by B or O or sentence boundary (DIFFERENT TYPE case)
        if curr_tag == 'I' and succ_tag in {'B', 'O', ''} and curr_type != succ_type:
            return 'I-{0}'.format(curr_type)
            # curr_tag == 'I':
            #    Preceeded by anything... (Pedantic: Only with SAME TYPE else it is Invalid)
            #    Succeeded by B or O or sentence boundary (SAME TYPE case)
        if curr_tag == 'I' and succ_tag in {'B', 'O', ''} and curr_type == succ_type:
            return 'E-{0}'.format(curr_type)
        if curr_tag == 'O':
            return 'O'
    ...
    if from_format == 'IOE1':
        ...
        if (curr_tag == 'I' and ((prec_tag in {'I', 'O', ''} and curr_type != prec_type) or (prec_tag == 'E' and curr_type == prec_type))
                and succ_tag in {'I', 'O', ''} and curr_type != succ_type):
            return 'I-{0}'.format(curr_type)
            # curr_tag == 'E':
            #    Preceeded by E with SAME TYPE
            #    Succeeded by SAME TYPE (I or E)
        if curr_tag == 'E' and ((prec_tag == 'E' and curr_type == prec_type) or curr_type != prec_type) and curr_type == succ_type:
            return 'E-{0}'.format(curr_type)
        ...
        if (curr_tag == 'I' and ((prec_tag == 'E' and curr_type == prec_type) or curr_type != prec_type) and
                curr_type == succ_type and succ_tag in {'I', 'E'}):
            return 'I-{0}'.format(curr_type)
        ...
        # Inside of chunk by tag
            ...
            #    Succeeded by ANY with SAME TYPE (I or E)
        if curr_tag == 'I' and prec_tag == 'I' and curr_type == prec_type and curr_type == succ_type and succ_tag in {'I', 'E'}:
            return 'I-{0}'.format(curr_type)
        ...
        if curr_tag == 'E' and prec_tag == 'I' and curr_type == prec_type and curr_type == succ_type:
            return 'E-{0}'.format(curr_type)
        if curr_tag == 'O':
            return 'O'
    ...
    if from_format == 'IOE2':
        # Single
            ...
            #    (DIFFERENT TYPE case)
        if curr_tag == 'E' and prec_tag in {'E', 'O', ''} and curr_type != succ_type:
            return 'I-{0}'.format(curr_type)
            # curr_tag == 'E':
            #    Preceeded by E or O with ANY TYPE or sentence boundary (as every end is marked with E)
            #    Succeeded by anything... (including sentence boundary) (as every end is marked with E) (SAME TYPE case)
        if curr_tag == 'E' and prec_tag in {'E', 'O', ''} and curr_type == succ_type:
            return 'E-{0}'.format(curr_type)
        ...
        # End of chunk by tag
            # curr_tag == 'E':
            #    Preceeded by E with SAME TYPE
            #    Succeeded by ANY TYPE (I or E) (including sentence boundary) (DIFFERENT TYPE case)
        if curr_tag == 'E' and prec_tag == 'I' and curr_type != succ_type:
            return 'I-{0}'.format(curr_type)
            # curr_tag == 'E':
            #    Preceeded by E with SAME TYPE
            #    Succeeded by ANY TYPE (I or E) (including sentence boundary) (SAME TYPE case)
        if curr_tag == 'E' and prec_tag == 'I' and curr_type == succ_type:
            return 'E-{0}'.format(curr_type)
        if curr_tag == 'O':
            return 'O'
    ...
    if from_format in {'IOBES', 'SBIEO'}:
        # Single chunk by tag
            # curr_tag == 'S':
            #    Preceeded by E or O with ANY TYPE or sentence boundary
            #    Succeeded by B or O with ANY TYPE or sentence boundary (DIFFERENT TYPE case)
        if curr_tag == 'S' and prec_tag in {'E', 'O', 'S', ''} and succ_tag in {'B', 'O', 'S', ''} and curr_type != succ_type:
            return 'I-{0}'.format(curr_type)
            # curr_tag == 'S':
            #    Preceeded by E or O with ANY TYPE or sentence boundary
            #    Succeeded by B or O with ANY TYPE or sentence boundary (SAME TYPE case)
        if curr_tag == 'S' and prec_tag in {'E', 'O', 'S', ''} and succ_tag in {'B', 'O', 'S', ''} and curr_type == succ_type:
            return 'E-{0}'.format(curr_type)
        ...
        # End of chunk by tag
            ...
            #    Preceeded by B or I with SAME TYPE
            #    Succeeded by B or O with ANY TYPE or sentence boundary (DIFFERENT TYPE case)
        if (curr_tag == 'E' and prec_tag in {'B', 'I'} and curr_type == prec_type and succ_tag in {'B', 'O', 'S', ''} and
                curr_type != succ_type):
            return 'I-{0}'.format(curr_type)
            # curr_tag == 'I':
            #    Preceeded by B or I with SAME TYPE
    ...
    if from_format == 'O+C':
        # Single chunk by tag
            ...
            #    Preceeded by E or O with ANY TYPE or sentence boundary
            #    Succeeded by B or O with ANY TYPE or sentence boundary (DIFFERENT TYPE case)
        if (curr_tag == 'S' and prec_tag in {'E', 'O', 'S', ''} and succ_tag in {'B', 'O', 'S', ''} and
                curr_type != succ_type and not inside):
            return 'I-{0}'.format(curr_type)
            # curr_tag == 'S':
            #    Preceeded by E or O with ANY TYPE or sentence boundary
            #    Succeeded by B or O with ANY TYPE or sentence boundary (SAME TYPE case)
        ...
        if (curr_tag == 'B' and prec_tag in {'E', 'O', 'S', ''} and ((succ_tag == 'E' and
                curr_type == succ_type) or succ_tag == 'O') and not inside):
            inside = True
            return 'I-{0}'.format(curr_type)
        ...
        # Inside of chunk by tag
            ...
            #    Succeeded by E with SAME TYPE (or O)
        if (curr_tag == 'O' and ((prec_tag == 'B' and curr_type == prec_type) or prec_tag == 'O') and
                ((succ_tag == 'E' and curr_type == succ_type) or succ_tag == 'O') and inside):
            return 'I-{0}'.format(curr_type)
        ...
        # End of chunk by tag
            ...
            #    Preceeded by B with SAME TYPE (or O)
            #    Succeeded by B or O with ANY TYPE or sentence boundary (DIFFERENT TYPE case)
        if (curr_tag == 'E' and ((prec_tag == 'B' and curr_type == prec_type) or prec == 'O') and
                succ_tag in {'B', 'O', 'S', ''} and curr_type != succ_type and inside):
            inside = False
            return 'I-{0}'.format(curr_type)
            # curr_tag == 'I':
            #    Preceeded by B with SAME TYPE (or O)
...
if __name__ == '__main__':
    ...
    for l in fh:
        ...
        if len(l) > 0:
            ...
            suc_label = ''
        if len(cur) > 0:
            print(''.join(cur), convert_to_ioe1(iob_format, prec_label, cur_label, suc_label))
        else:
            print()
