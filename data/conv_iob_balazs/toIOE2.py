...
def convert_to_ioe2(from_format, prec, curr, succ):
    global inside
    ...
    if from_format == 'IOE2':
        ...
        # Begin of chunk by tag
            ...
            #    Succeeded by I or E with SAME TYPE (as every end is marked with E)
        if curr_tag == 'I' and prec_tag in {'E', 'O', ''} and succ_tag in {'I', 'E'} and curr_type == succ_type:
            return 'I-{0}'.format(curr_type)

        # Inside of chunk by tag
            # curr_tag == 'I':
            #    Preceeded by I (as every end is marked with E) (Pedantic: SAME TYPE)
            #    Succeeded by TYPE (I or E) (as every end is marked with E) (Pedantic: SAME TYPE)
        if curr_tag == 'I' and prec_tag == 'I' and curr_type == prec_type and succ_tag in {'I', 'E'} and curr_type == succ_type:
            return 'I-{0}'.format(curr_type)
        ...
        if curr_tag == 'E' and prec_tag == 'I' and curr_type == prec_type:
            return 'E-{0}'.format(curr_type)
        if curr_tag == 'O':
            return 'O'
    ...
    if from_format in {'IOBES', 'SBIEO'}:
        # Single chunk by tag
            # curr_tag == 'S':
            #    Preceeded by ANY with ANY TYPE or sentence boundary
            #    Succeeded by ANY with ANY TYPE or sentence boundary
        if curr_tag == 'S':
            return 'E-{0}'.format(curr_type)
        ...
        # Begin of chunk by tag
            # curr_tag == 'B':
            #    Preceeded by ANY with ANY TYPE or sentence boundary
            #    Succeeded by ANY with SAME TYPE
        if curr_tag == 'B':
            return 'I-{0}'.format(curr_type)

        # Inside of chunk by tag
            # curr_tag == 'I':
            #    Preceeded by ANY with SAME TYPE
            #    Succeeded by ANY with SAME TYPE
        if curr_tag == 'I':
            return 'I-{0}'.format(curr_type)
...
if __name__ == '__main__':
    ...
    for l in fh:
        ...
        if len(l) > 0:
            ...
            suc_label = ''
        if len(cur) > 0:
            print(''.join(cur), convert_to_ioe2(iob_format, prec_label, cur_label, suc_label))
        else:
            print()
